...
> 💡 **Быстрая справка**: См. [QUICK_REFERENCE.md](QUICK_REFERENCE.md) для кратких команд и ссылок
...
Flower предоставляет красивый веб-интерфейс для мониторинга Celery:

```bash
# Flower запускается автоматически
...
- ⏱️ История выполнения задач
- 🔄 Управление задачами (retry, revoke)
...
## 🌸 Flower - Мониторинг Celery в Реальном Времени

**Flower** - это веб-инструмент для мониторинга и администрирования Celery кластеров. Предоставляет красивый и удобный интерфейс для отслеживания задач.
...
### Как Использовать Flower

```bash
# Flower запускается автоматически с docker-compose
docker compose up -d

# Откройте в браузере
open/open_command http://localhost:5555
```

### Примеры Использования

**Просмотр активных задач:**
1. Отправьте несколько задач через веб-интерфейс
2. Откройте Flower → Tasks
3. Увидите все задачи с их статусами

**Мониторинг производительности:**
1. Откройте Flower → Dashboard
2. Наблюдайте графики в реальном времени
3. Отслеживайте загрузку worker'ов

**Управление задачами:**
1. Найдите задачу в списке Tasks
2. Нажмите на ID задачи для деталей
3. Можете отменить (Revoke) или повторить (Retry) задачу

### Полезные Метрики в Flower

- **Succeeded** - успешно выполненные задачи
- **Failed** - задачи с ошибками
- **Retried** - повторно запущенные задачи
- **Active** - задачи в процессе выполнения
- **Task runtime** - время выполнения задач
...
- **[CELERY_GUIDE.md](CELERY_GUIDE.md)** - Подробное руководство по Celery
- **[FLOWER_GUIDE.md](FLOWER_GUIDE.md)** - Руководство по мониторингу с Flower
