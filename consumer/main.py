...
def main():
    ...
    def callback(ch, method, properties, body):
        ...
        try:
            ...
            requests.post(API_URL, json={'task_num': task_num, 'task_status': 'IN_PROGRESS'})
            time.sleep(int(data.get('timeout', 0)))
            ch.basic_ack(delivery_tag=method.delivery_tag)
            requests.post(API_URL, json={'task_num': task_num, 'task_status': 'DONE'})
    ...
    channel.basic_consume(queue=QUEUE_NAME, on_message_callback=callback, auto_ack=False, consumer_tag='test_rmq')
